...
const { HOUR_MS, toIsoHour, buildMissingIntervals } = require("./series-utils");
...
const floorToResolutionIso = (dateLike, resolutionMinutes) => {
  ...
  const size = Math.max(1, Number(resolutionMinutes) || 60);
...
const buildRangeByResolution = (start, end, resolutionMinutes) => {
  const list = [];
  const stepMs = Math.max(1, Number(resolutionMinutes) || 60) * 60 * 1000;
  const firstIso = floorToResolutionIso(start, resolutionMinutes);
  const lastIso = floorToResolutionIso(end, resolutionMinutes);
  const cursor = new Date(firstIso);
...
const inferResolutionFromRows = (rows = [], fallback = 60) => {
...
const parseCaisoRows = (rows = [], { marketMode = "day_ahead" } = {}) =>
  rows
    .map((row) => {
      const ts =
        ...
        row.datetime ||
        "";
      // Day-ahead must only parse explicit price fields.
      // Falling back to non-price fields (for example MW) can falsely persist missing hours as zero prices.
      // Price series must only parse explicit LMP price fields.
      // Falling back to non-price fields (for example MW) can persist missing intervals as false zeros.
      const value = toNumber(row.lmp) ?? toNumber(row.lmp_prc);
      const parsed = new Date(ts);
      if (!Number.isFinite(value) || Number.isNaN(parsed.getTime())) return null;
      const resolutionMinutes = marketMode === "real_time" ? 5 : 60;
      return { ts: floorToResolutionIso(parsed, resolutionMinutes), value };
    })
    .filter(Boolean);

const MAX_CAISO_WINDOW_MS = 31 * 24 * HOUR_MS;

const parseChunkHours = (value, fallback, { min = 1, max = 24 * 31 } = {}) => {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return fallback;
  return Math.max(min, Math.min(max, numeric));
};

const resolveCaisoChunkHours = ({ marketMode, chunkProfile = "auto" }) => {
  const normalizedMode = String(marketMode || "day_ahead").toLowerCase();
  const normalizedProfile = String(chunkProfile || "auto").toLowerCase();

  if (normalizedMode === "real_time") {
    return parseChunkHours(process.env.ENERGYAPP_CAISO_RT_CHUNK_HOURS, 4, { min: 2, max: 6 });
  }

  if (normalizedProfile === "backfill") {
    return parseChunkHours(process.env.ENERGYAPP_CAISO_DA_BACKFILL_CHUNK_HOURS, 72, { min: 24, max: 31 * 24 });
  }

  return parseChunkHours(process.env.ENERGYAPP_CAISO_DA_VISIBLE_CHUNK_HOURS, 24, { min: 24, max: 31 * 24 });
};

const resolveCaisoConcurrency = ({ marketMode }) => {
  const perModeEnv =
    String(marketMode || "") === "real_time"
      ? process.env.ENERGYAPP_CAISO_RT_CONCURRENCY
      : process.env.ENERGYAPP_CAISO_DA_CONCURRENCY;
  const fallback = process.env.ENERGYAPP_CAISO_MAX_CONCURRENCY;
  const requested = Number(perModeEnv || fallback || 2);
...
const buildCaisoChunks = ({ start, end, marketMode = "day_ahead", chunkProfile = "auto" }) => {
  const chunks = [];
  ...
  const chunkHours = resolveCaisoChunkHours({ marketMode, chunkProfile });
  const maxChunkMs = chunkHours * HOUR_MS;

  let cursor = new Date(rawStart);
  while (cursor < cappedEnd) {
    const chunkStart = new Date(cursor);
    const chunkEnd = new Date(Math.min(cappedEnd.getTime(), chunkStart.getTime() + maxChunkMs));
    chunks.push({ start: chunkStart, end: chunkEnd, profile: chunkProfile, marketMode });
...
const fetchCaisoChunkLive = async ({ start, end, marketMode, node }) => {
  ...
  for (let i = 0; i < attempts.length; i += 1) {
    ...
    if (csv) {
      return {
        rows: parseCaisoRows(parseCsvRows(csv), { marketMode }),
        sourceUrl: url.toString(),
      };
    }
...
const fetchCaisoLive = async ({ start, end, marketMode, lat, utilityCode, chunkProfile = "auto" }) => {
  const node = resolveCaisoNode({ lat, utilityCode });
  const chunks = buildCaisoChunks({ start, end, marketMode, chunkProfile });
  const mergedRows = [];
  let lastError = null;
  let firstSourceUrl = "";

  const concurrency = resolveCaisoConcurrency({ marketMode });
  let nextIndex = 0;
  ...
  const workers = Array.from({ length: Math.max(1, Math.min(concurrency, chunks.length || 1)) }, () => worker());
...
const parseErcotRows = (rows = [], { marketMode = "day_ahead" } = {}) =>
  rows
    .map((row) => {
      ...
      const resolutionMinutes = marketMode === "real_time" ? 15 : 60;
      return { ts: floorToResolutionIso(parsed, resolutionMinutes), value };
    })
    .filter(Boolean);
...
const fetchErcotLive = async ({ marketMode, start, end, lat, lng, utilityCode }) => {
  ...
  const parsedRows = parseErcotRows(rows, { marketMode });
...
const mapAverageByIsoBucket = (rows = [], resolutionMinutes = 60) => {
...
const buildSeriesFromRows = ({
  ...
  const now = Date.now();
  const byBucket = mapAverageByIsoBucket(rows, resolutionMinutes);
  const publishHorizonMs = marketMode === "real_time" ? 6 * HOUR_MS : 72 * HOUR_MS;
  const points = buildRangeByResolution(start, end, resolutionMinutes).map((date) => {
    const ts = floorToResolutionIso(date, resolutionMinutes);
...
const buildUnavailableSeries = ({
  start,
  end,
  marketMode,
  reason = "source_unavailable",
  source = "rates_proxy_phase3_live_unavailable",
  message = "No rate data from source.",
  resolutionMinutes = 60,
}) => {
  const now = Date.now();
  const points = buildRangeByResolution(start, end, resolutionMinutes).map((date) => {
...
const fetchLiveRowsByRegion = async ({ regionId, marketMode, start, end, lat, lng, utilityCode, chunkProfile = "auto" }) => {
  if (regionId === "CAISO") {
    const result = await fetchCaisoLive({ start, end, marketMode, lat, utilityCode, chunkProfile });
    return {
      rows: result.rows,
...
const getLmpSeries = async ({ regionId, marketMode, start, end, lat, lng, utilityCode, chunkProfile = "auto" }) => {
  const capabilities = ADAPTER_CAPABILITIES[regionId] || { real_time: false, day_ahead: false };
  const fallbackResolutionMinutes = marketMode === "real_time" ? 5 : 60;
  if (!capabilities[marketMode]) {
    return buildUnavailableSeries({
      ...
      source: "rates_proxy_phase3_region_unsupported",
      message: "Rates in this region are not supported.",
      resolutionMinutes: fallbackResolutionMinutes,
    });
  }
  try {
    ...
    const resolutionMinutes = inferResolutionFromRows(rows, fallbackResolutionMinutes);
    ...
    const fallback = buildUnavailableSeries({
      start,
      end,
      marketMode,
      reason: "source_unavailable",
...
module.exports = {
  getLmpSeries,
  __internal: {
    inferResolutionFromRows,
    buildUnavailableSeries,
    buildSeriesFromRows,
    parseCaisoRows,
    buildCaisoChunks,
    resolveCaisoChunkHours,
    resolveCaisoConcurrency,
  },
};
