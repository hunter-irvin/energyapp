...
const extractCaisoErrorDescription = (buffer) => {
  ...
  const desc = String(descMatch?.[1] || "").trim();
  if (code || desc) return `${code}${code && desc ? ": " : ""}${desc}`.trim();
  return "";
};
...
const fetchCaisoLive = async ({ start, end, marketMode }) => {
  ...
  let lastErrorMessage = "CAISO returned no CSV payload";
  ...
  for (let i = 0; i < attempts.length; i += 1) {
    ...
    if (upstream.statusCode < 200 || upstream.statusCode >= 300) {
      lastErrorMessage = `CAISO upstream status ${upstream.statusCode}`;
      continue;
    }
    ...
    const errorDescription = extractCaisoErrorDescription(upstream.body);
    lastErrorMessage = errorDescription
      ? `CAISO returned no CSV payload (${errorDescription})`
