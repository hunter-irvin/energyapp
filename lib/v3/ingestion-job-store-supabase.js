  ...
  return {
    ...
    async readRateSeriesWindow({ projectId, serviceType, marketMode, windowStart, windowEnd, resolutionMinutes }) {
      const rows = await rest({
        ...
        searchParams: {
          select: "ts,resolution_minutes,value",
          project_id: `eq.${projectId}`,
      ...
      const endIso = String(windowEnd || "");
      return (Array.isArray(rows) ? rows : []).filter((row) => String(row.ts || "") <= endIso);
    },
    ...
    async upsertRateSyncChunks(rows = []) {
      ...
      for (const batch of batches) {
        ...
        await rest({
          ...
          searchParams: {
            on_conflict: "job_id,project_id,service_type,market_mode,chunk_start,chunk_end",
          },
          headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
          body: batch,
        });
    ...
    async updateProjectRatesFingerprint(projectId, fingerprint) {
      await rest({
        ...
        body: {
          rates_source_fingerprint: fingerprint || null,
          updated_at: new Date().toISOString(),
        },
      });
      return true;
