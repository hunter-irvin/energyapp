...
const runLoadBuilderStaticTests = () => {
  ...
  assert.ok(pageScript.includes("onDeleteProfile"), "Load Builder should wire profile delete actions.");
  assert.ok(pageScript.includes("supabaseService.deleteLoadProfile"), "Load Builder should persist profile deletion.");
  assert.ok(pageScript.includes("onRenameRow"), "Load Builder should wire layer rename actions.");
  assert.ok(pageScript.includes("requestAssistantTurn"), "Load Builder should call the AI assistant endpoint.");
  assert.ok(pageScript.includes("/api/load-profile-assistant"), "Load Builder should route AI turns through the shared assistant API.");
  assert.ok(pageScript.includes("createProfileModelFromAssistantProposal"), "Load Builder should interpret assistant proposals into profile models.");
  assert.ok(pageScript.includes("onApplyAssistantProposal"), "Load Builder should support replacing editor loads from an assistant proposal.");
  assert.ok(pageScript.includes("supabaseService.upsertLoadProfile"), "Load Builder should persist AI-created and AI-updated profiles.");
  assert.ok(/applyAssistantProposal[\s\S]*scheduleAutosave\(\)/.test(pageScript), "Editor AI replacement should schedule autosave.");
  assert.ok(/restoreProfiles[\s\S]*validateProfileModel/.test(pageScript), "Reload should restore persisted AI profile models through validation.");
  ...
  assert.ok(uiScript.includes("ProfileDeleteButton"), "Load Profiles landing should expose profile deletion.");
  assert.ok(uiScript.includes("load-builder-profile-preview-actions"), "Profile delete should sit with the aggregate preview actions.");
  ...
  assert.ok(uiScript.includes("rows: aggregateRows"), "Aggregate chart should use its own stack order.");
  assert.ok(uiScript.includes("legendRows = rows"), "Legend should follow row list order instead of reversed stack order.");
  assert.ok(uiScript.includes("onToggleRowMuted"), "Load Builder UI should expose layer hide/show toggles.");
  assert.ok(uiScript.includes("eye-off") && uiScript.includes("Hide layer") && uiScript.includes("Show layer"), "Layer actions should expose eye hide/show icons.");
  ...
  assert.ok(uiScript.includes("Generate with AI"), "New profile modal should expose the AI intake option.");
  assert.ok(uiScript.includes("Start with that"), "AI intake should allow skipping clarifying questions.");
  assert.ok(uiScript.includes("ASSISTANT_FACILITY_TYPES"), "AI intake should define facility type choices.");
  assert.ok(uiScript.includes("Residential") && uiScript.includes("Commercial") && uiScript.includes("Industrial"), "AI intake should show residential/commercial/industrial choices.");
  assert.ok(uiScript.includes("ASSISTANT_MAJOR_LOAD_OPTIONS"), "AI intake should define upfront major-load checklist choices.");
  assert.ok(uiScript.includes("Clothes dryer"), "AI intake checklist should include clothes dryer.");
  assert.ok(uiScript.includes("none_of_these"), "AI intake checklist should include an explicit none-of-these option.");
  assert.ok(uiScript.includes("load-builder-modal__header"), "New profile modal should render title, progress, and cancel in the header.");
  assert.ok(uiScript.includes("Please describe any nuances about your daily electricity use"), "AI intake should frame the text field as daily-use nuance.");
  assert.ok(uiScript.includes("selectionType") && uiScript.includes("load-builder-ai-options--multiple"), "AI intake should support multiselect assistant questions.");
  assert.ok(uiScript.includes("load-builder-ai-review"), "AI intake should include a final concise review screen.");
  assert.ok(uiScript.includes("onOpenAiGen"), "Editor library should expose an AI Gen entry point.");
  assert.ok(uiScript.includes("load-builder-row-info-action"), "AI-generated rows should expose their reason text.");
};
